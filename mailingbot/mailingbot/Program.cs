         ...
         try {
            try {
               if (message.Text == "/mychanels") {
                  ...
                  try {
                     var user = users.Find(x => x.id.ToString() == message.Chat.Id.ToString());
                     if (user == null) {
                        ConnectDB.Query("insert into `User` (id, first_name) values ('" + message.From.Id + "', '" + message.From.FirstName + "');");
                     }
                     await client.SendTextMessageAsync(message.Chat.Id, "*Добро пожаловать*\n\nДанный бот позволяет формировать базу пользователей со своих каналов и создавать по ней рассылку.\nКоманды бота:\n/mychanels - ваши каналы;\n/mailing - создание рассылки по базе.", Telegram.Bot.Types.Enums.ParseMode.Markdown);
                  } catch {
                     ConnectDB.Query("insert into `User` (id, first_name) values ('" + message.From.Id + "', '" + message.From.FirstName + "');");
                     await client.SendTextMessageAsync(message.Chat.Id, "*Добро пожаловать*\n\nДанный бот позволяет формировать базу пользователей со своих каналов и создавать по ней рассылку.\nКоманды бота:\n/mychanels - ваши каналы;\n/mailing - создание рассылки по базе.", Telegram.Bot.Types.Enums.ParseMode.Markdown);
                  }
